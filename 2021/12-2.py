def main():
    ...
    paths = []
    walk_graph(start, [], paths, False)

    # for path in paths:  # Almost all of this program's run time is just from printing the paths.
    #     print(','.join(path))
    print(f'\n{len(paths) = }')  # 120506
...
def walk_graph(node: list,
               ...
               revisited_small_cave: bool) -> None:
    ...
    for next_node in node[1:]:
        if next_node[0].isupper() or next_node[0] not in _this_path:
            walk_graph(next_node, _this_path, all_paths, revisited_small_cave)
        elif not revisited_small_cave and next_node[0] != 'start':
            walk_graph(next_node, _this_path, all_paths, True)
